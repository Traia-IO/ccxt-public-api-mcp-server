...
# Script to build and run the CCXT Public API MCP Server locally in Docker
...
IMAGE_NAME="ccxt-public-api-mcp-server"
CONTAINER_NAME="ccxt-public-api-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running CCXT Public API MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# CCXT Public API MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ CCXT Public API MCP Server is running!${NC}"
echo
