# CCXT Public API MCP Server Implementation Guide

You are working on implementing the CCXT Public API MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: CCXT Public API
- **Documentation**: [https://docs.ccxt.com](https://docs.ccxt.com)
- **Website**: [https://api.ccxt.com](https://api.ccxt.com)
- **SDK**: `ccxt` (already included in pyproject.toml)
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_ccxt_public_api",
      "get_ccxt_public_api_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["ccxt public api", "api", /* add relevant tags like "search", "data", etc. */]
}
```

### 2. Study the API Documentation

First, thoroughly review the API documentation at https://docs.ccxt.com to understand:
- Available endpoints
...
Add functions to call the CCXT Public API API with retry support. Example pattern:
...
def call_ccxt_public_api_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call CCXT Public API API using the SDK with automatic retry."""
    # Initialize the SDK client
    client = ccxt.Client(api_key=api_key)
    ...
    try:
        ...
        raise Exception(f"CCXT Public API API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_ccxt_public_api(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search CCXT Public API for [specific data type].
    ...
    try:
        # The call_ccxt_public_api_api function already has retry logic
        results = call_ccxt_public_api_api(
...
```python
@mcp.tool()
async def get_ccxt_public_api_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_ccxt_public_api_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in CCXT Public API.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_ccxt_public_api(query="example", limit=5)
   ```
...
### 9. Common CCXT Public API Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on CCXT Public API capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the CCXT Public API documentation that you should implement:
...
- Check the CCXT Public API API documentation: https://docs.ccxt.com
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make CCXT Public API's capabilities easily accessible to AI agents through standardized MCP tools. 